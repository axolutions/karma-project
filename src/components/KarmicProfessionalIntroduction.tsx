...
import { INTRODUCTION } from "@/lib/default-interpretations-professional"

interface KarmicProfessionalIntroductionProps {
  userName?: string
}

const KarmicProfessionalIntroduction: React.FC<KarmicProfessionalIntroductionProps> = ({ userName }) => {
  ...
  const formatIntroduction = () => {
    ...
    return paragraphs.map((paragraph, index) => {
      ...
      // Format headings
      if (paragraph.includes("Mapa Kármico Profissional 2025") && index === 0) {
        return (
      ...
      // Format disclaimer
      if (paragraph.includes("A Matriz Kármica Profissional é criada com carinho")) {
        return (
      ...
      <div className="bg-[#f5f5dc] rounded-lg shadow-sm overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-[#e2d1c3]">
          <h3 className="text-xl text-[#333333] font-semibold flex items-center">
            ...
            Bem-vindo{userName ? `, ${userName}` : ""} à sua Matriz Kármica Profissional
          </h3>
        </div>
...
export default KarmicProfessionalIntroduction
