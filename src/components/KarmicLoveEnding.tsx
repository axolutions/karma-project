...
import { ENDING } from "@/lib/default-interpretations-love"

interface KarmicLoveEndingProps {
  userName?: string
}

const KarmicLoveEnding: React.FC<KarmicLoveEndingProps> = ({ userName }) => {
  ...
  const formatEnding = () => {
    ...
    return paragraphs.map((paragraph, index) => {
      ...
      // Format main heading
      if (paragraph.includes("Conclusão do Mapa Kármico do Amor 2025")) {
        return (
      ...
      // Format subheading
      if (paragraph.includes("Agora que você conhece seus códigos kármicos do amor")) {
        return (
      ...
      if (paragraph.includes("🔥") || paragraph.includes("✨ Sua Jornada Amorosa Começa Agora! ✨") || paragraph.includes("❤️")) {
        return (
      ...
      if (
        paragraph.includes("O Mapa Kármico do Amor 2025 não é apenas um guia") ||
      ...
      <div className="bg-[#f5f5dc] rounded-lg shadow-sm overflow-hidden">
        <div className="flex justify-between items-center p-4 border-b border-[#e2d1c3]">
          <h3 className="text-xl text-[#333333] font-semibold flex items-center">
            ...
            Próximos Passos na Sua Jornada Kármica Amorosa
          </h3>
        </div>
...
export default KarmicLoveEnding
