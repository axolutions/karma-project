...
import { ENDING } from "@/lib/default-interpretations"

interface KarmicEndingProps {
  userName?: string
}

const KarmicEnding: React.FC<KarmicEndingProps> = ({ userName }) => {
  ...
  const formatEnding = () => {
    ...
    return paragraphs.map((paragraph, index) => {
      ...
      // Format main heading
      if (paragraph.includes("Conclusão do Mapa Kármico Pessoal 2025")) {
        return (
      ...
      // Format subheading
      if (paragraph.includes("Agora que você conhece seus códigos kármicos")) {
        return (
      ...
      if (paragraph.includes("🔥") || paragraph.includes("✨ Sua Jornada Começa Agora! ✨")) {
        return (
      ...
      if (
        paragraph.includes("O Mapa Kármico Pessoal 2025 não é apenas um guia") ||
