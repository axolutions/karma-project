...
const EmailManager: React.FC = () => {
  ...
  const [tempMapsAvailable, setTempMapsAvailable] = useState<string[] | null>(
    null,
  );
  ...
  const filteredEmails = emails.filter((email) => {
    const matchesFilter = !mapFilter ||
      email.maps_available?.includes(mapFilter);
  ...
  const handleChangeMapChoosen = async (
    email: string,
    mapChoosen: string | null,
  ) => {
    try {
      ...
      if (success) {
        toast.success("Mapa atualizado", {
          description:
            `O tipo de mapa para ${email} foi atualizado com sucesso.`,
        });
        refreshEmails();
      } else {
  ...
  const handleToggleMap = async (
    email: string,
    mapType: string,
    currentMapsAvailable: string[] | null,
    currentMapChoosen: string | null,
  ) => {
    try {
      ...
      const success = await updateMapChoosen(
        email,
        newMapChoosen,
        mapsAvailable,
      );
  ...
  const handleSaveMapChanges = async () => {
    ...
    try {
      ...
      if (success) {
        toast.success("Mapas atualizados", {
          description:
            `Os mapas para ${selectedEmail} foram atualizados com sucesso.`,
        });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-end gap-3">
        <div className="flex-1">
          <label
            htmlFor="email"
            className="text-base font-medium text-karmic-700 block mb-2"
          >
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="personal" className="text-base">
                  Pessoal
                </SelectItem>
                <SelectItem value="professional" className="text-base">
                  Profissional
                </SelectItem>
      ...
      {emails.length === 0 && (
        <div className="p-4 bg-amber-50 border border-amber-200 rounded-md">
          <h3 className="text-amber-800 text-base font-medium mb-2">
            Nenhum email cadastrado
          </h3>
          <p className="text-amber-700 text-sm mb-3">
            Você precisa adicionar pelo menos um email autorizado para que os
            usuários possam fazer login.
          </p>
          <Button
          ...
          >
            ...
            Adicionar email de exemplo (projetovmtd@gmail.com)
          </Button>
        </div>
      )}
      ...
      <div>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center">
              ...
              <Select
                value={mapFilter || "all"}
                onValueChange={(value) =>
                  setMapFilter(value === "all" ? null : value)}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="love" className="text-base">
                    Amor
                  </SelectItem>
                  <SelectItem value="personal" className="text-base">
                    Pessoal
        ...
        {filteredEmails.length === 0
          ? (
            <p className="text-karmic-500 italic text-base">
              Nenhum email autorizado encontrado com os filtros aplicados.
            </p>
          )
          : (
            <>
                    ...
                    return (
                      <li
                        ...
                        className={`flex justify-between items-center p-4 rounded-md ${
                          essential ? "bg-blue-50" : "bg-karmic-100"
                        }`}
                      >
                        <div className="flex items-center flex-wrap gap-2">
                          <span
                            className={`${
                              essential ? "font-medium text-blue-700" : ""
                            } text-lg`}
                          >
                            {email}
                          </span>
                            ...
                            <div className="ml-1 flex items-center">
                              <div className="flex items-center text-sm bg-karmic-200 text-karmic-700 px-2 py-1 rounded-full">
                                ...
                                <span className="ml-1 font-medium">
                                  ...
                                  {map_choosen && (
                                    <span className="ml-1">
                                      (Atual: {map_choosen === "professional"
                                        ? "Profissional"
                                        : map_choosen === "love"
                                        ? "Amor"
                                        : "Pessoal"})
                            ...
                            >
                              <div className="w-72">
                                ...
                                <div className="space-y-3 mb-4">
                                  {["professional", "love", "personal"].map(
                                    (mapType) => {
                                      const mapName = mapType === "professional"
                                        ? "Profissional"
                                        : mapType === "love"
                                        ? "Amor"
                                        : "Pessoal";
                                      ...
                                      return (
                                        <label
                                          key={mapType}
                                          className="flex items-center space-x-3 cursor-pointer text-base"
                                        >
                                          <input
                                            type="checkbox"
                                          ...
                                          <span>{mapName}</span>
                                          {isCurrentMapChosen && (
                                            <span className="text-sm text-karmic-500">
                                              (Atual)
                                            </span>
                                          )}
                                        </label>
                        ...
                        <Button
                          ...
                          className={`hover:bg-transparent ${
                            essential
                              ...
                              : "text-karmic-600 hover:text-red-500"
                          }`}
                          onClick={() => !essential && setEmailToDelete(email)}
                          disabled={essential}
                          title={essential
                            ? "Este email não pode ser removido"
                            : "Remover email"}
                        >
                          <X className="h-5 w-5" />
                        </Button>
      ...
      >
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
            <DialogTitle className="text-xl text-karmic-800">
              Confirmar exclusão
            </DialogTitle>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-md p-4 mt-2 flex items-start gap-3">
            ...
            <div className="text-base text-amber-800">
              O usuário perderá acesso ao seu mapa kármico criado com este
              email. Esta ação não pode ser desfeita.
            </div>
          </div>
      ...
      <div className="mt-4 p-4 bg-karmic-50 border border-karmic-200 rounded-md">
        <h4 className="text-base font-medium text-karmic-700 mb-2">
        ...
        <ul className="text-sm space-y-2 text-karmic-600 list-disc pl-4">
          ...
          <li>
            Para conceder acesso a um novo mapa, adicione o mesmo email
            novamente
          </li>
          <li>
            Quando um email é adicionado novamente, ele recebe permissão para
            criar um novo mapa
          </li>
          <li>
            Remover um email impedirá que o usuário acesse todos os mapas
            criados com esse email
          </li>
          <li className="text-blue-600 font-medium">
            Os emails essenciais do sistema (marcados em azul) não podem ser
            removidos
          </li>
        </ul>
      </div>
