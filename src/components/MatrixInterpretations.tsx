...
const MatrixInterpretations: React.FC<MatrixInterpretationsProps> = ({
	karmicData,
}) => {
	...
	useEffect(() => {
		...
		try {
			...
			console.log(
				"MatrixInterpretations: Interpretações carregadas com sucesso",
			);
		} catch (err) {
			...
			try {
				...
				setLoadError(null);
				console.log(
					"MatrixInterpretations: Fallback para interpretações de amostra carregado com sucesso",
				);
			} catch (e) {
				console.error(
		...
		return (
			<div className="max-w-4xl mx-auto mt-8 p-8 bg-red-50 border border-red-200 rounded-md text-center">
				...
				<h2 className="text-xl font-medium text-red-600 mb-2">
					Erro ao carregar interpretações
				</h2>
				<p className="text-red-500">
					Não foi possível carregar os dados das interpretações kármicas.
				</p>
		...
		return (
			<div className="max-w-4xl mx-auto mt-8 p-8 bg-yellow-50 border border-yellow-200 rounded-md text-center">
				...
				<p className="text-sm text-yellow-500 mt-2">
					Algumas interpretações podem não estar disponíveis.
				</p>
			</div>
		);
	}
		...
		return (
			<div className="max-w-4xl mx-auto mt-8 p-8 bg-gray-50 border border-gray-200 rounded-md text-center">
				...
				<p className="text-gray-500">
					Aguarde enquanto carregamos suas interpretações kármicas.
				</p>
			</div>
		);
	}
			...
			<div className="space-y-2">
				{interpretationItems.map((item, index) => {
					...
					console.log(
						...
						interpretation
							? `Conteúdo: ${interpretation.content?.substring(0, 50)}...`
							: "Interpretação vazia",
					);
						...
						>
							...
							>
								<h3 className="text-lg text-[#333333] font-semibold">
									...
									{interpretation?.title === "NÃO EXISTO"
										? "Não encontrado"
							...
							<AnimatePresence>
								{isExpanded && (
									...
									>
										<div className="p-4 pt-0 border-t border-[#e2d1c3]">
											{interpretation && interpretation.content ? (
												<div
													className="prose max-w-none text-[#333333]"
													dangerouslySetInnerHTML={{
														__html: interpretation.content,
													}}
												/>
											) : (
												<p className="text-[#8B4513] italic">
													Interpretação não disponível para{" "}
													{getCategoryDisplayName(item.key)} com valor{" "}
													{item.value}.
												</p>
											)}
										</div>
